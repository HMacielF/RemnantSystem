...
def to_unified(record: UmiNaturalStoneRecord, scraped_at: str) -> UnifiedSlabRecord:
    width_in, height_in = parse_dimensions_inches(record.size)
    extra = {}
    if record.product_category:
        extra["product_category"] = record.product_category
...
def export_records(records: list[UmiNaturalStoneRecord], output_dir: Path, category_key: str) -> tuple[Path, Path]:
    ...
    csv_path = export_unified_csv(unified, output_dir, supplier="umi_natural_stones", suffix=f"beltsville_{slug}")
