...
export function MySoldPanel({
 ...
 <h2 className="font-inter mt-1 text-2xl font-semibold text-[color:var(--qc-ink-1)]">Sold Remnants</h2>
 ...
 return (
 <article key={sale.id} className="rounded-sm border border-[color:var(--qc-line)] bg-[linear-gradient(180deg,#ffffff_0%,#fafafa_100%)] p-4 ">
 ...
 <p className="mt-2 text-sm font-medium text-[color:var(--qc-ink-1)]">{formatDateLabel(sale.sold_at || sale.created_at)}</p>
 </div>
 <div className="rounded-sm border border-[color:var(--qc-line)] bg-white p-4">
 <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--brand-orange)]">Job</p>
 <p className="mt-2 text-sm font-medium text-[color:var(--qc-ink-1)]">{sale.job_number ? formatJobNumber(sale.job_number, soldRemnant) : "Unknown"}</p>
 </div>
 </div>
