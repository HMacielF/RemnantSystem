...
export function PrivateRemnantSummaryBlock({ remnant, className = "", onOpenImage }) {
 ...
 return (
 <section
 className={`rounded-sm border border-[color:var(--qc-line)] bg-white p-3.5 sm:p-4 ${className}`.trim()}
 >
 ...
 onClick={onOpenImage}
 disabled={!image}
 className={`overflow-hidden rounded-sm border border-white/80 bg-white text-left transition-transform ${image ? "hover:-translate-y-0.5" : "cursor-default"}`}
 >
 {image ? (
 <img
 alt={`Remnant ${displayId}`}
 className="h-36 w-full bg-[color:var(--qc-bg-page)] p-2 object-contain object-center"
 src={image}
 />
 ) : (
 <div className="flex h-36 w-full items-center justify-center bg-[color:var(--qc-bg-page)] px-6 text-center text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--brand-orange)]">
 No Image
 </div>
 )}
 </button>
 ...
 <div className="mt-3">
 <h3 className="font-inter text-lg font-semibold text-[color:var(--qc-ink-1)] sm:text-xl">
 ...
 <span className="inline-flex rounded-full border border-[color:var(--qc-line)] bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--brand-orange-deep)]">
 ID #{displayId}
 </span>
 ...
 className="rounded-sm border border-[color:var(--qc-line)] bg-white/92 px-4 py-3"
 >
 <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--brand-orange)]" title={entry.title}>
 {entry.label}
 </p>
 <p className="mt-1 text-sm font-medium text-[color:var(--qc-ink-1)]">{entry.value}</p>
 </div>
 ))}
 </div>
 </div>
...
export function SelectField({ wrapperClassName = "relative mt-2", className = "", children, ...props }) {
 ...
 <select
 {...props}
 className={`h-12 w-full appearance-none rounded-sm border border-[color:var(--qc-line)] bg-white px-4 pr-10 text-sm font-medium text-[color:var(--qc-ink-1)] outline-none transition-colors focus:border-[var(--brand-orange)] focus:ring-4 focus:ring-[rgba(247,134,57,0.14)] ${className}`}
 >
 {children}
 </select>
 <svg
...
export function InAppSelect({
 ...
 disabled={disabled}
 className={`flex h-12 w-full items-center justify-between gap-3 rounded-sm border border-[color:var(--qc-line)] bg-white px-4 text-left text-sm font-medium outline-none transition-colors focus:border-[var(--brand-orange)] focus:ring-4 focus:ring-[rgba(247,134,57,0.14)] ${selected ? "text-[color:var(--qc-ink-1)]" : "text-[rgba(35,35,35,0.48)]"} ${disabled ? "cursor-not-allowed opacity-60" : ""}`}
 >
 ...
 className={`flex w-full items-center rounded-sm px-3 py-2.5 text-left transition-colors hover:bg-[color:var(--qc-bg-page)] ${isSelected ? "bg-[color:var(--qc-bg-page)]" : ""}`}
 >
 <span className={`block truncate text-sm ${isSelected ? "font-semibold text-[color:var(--qc-ink-1)]" : "font-medium text-[rgba(35,35,35,0.78)]"}`}>
 {option.label}
 </span>
 </button>
 );
...
export function PrivateWorkspaceSkeletonCard({ showActions = false }) {
 return (
 <div className="overflow-hidden rounded-sm border border-white/20 bg-white/16 ">
 ...
 <div className="space-y-3 rounded-sm bg-white/85 px-3 py-3">
 ...
 <div key={index} className="h-11 animate-pulse rounded-sm bg-white/18" />
 ))}
 </div>
 ) : null}
 </div>
